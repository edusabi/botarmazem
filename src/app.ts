...
async function startBot() {
  ...
  sock.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      console.log(
        `⚠️ Conexão fechada (${statusCode}). Reconectar? ${shouldReconnect}`
      );
      ...
      if (shouldReconnect) {
        ...
        console.log(
          "Sessão deslogada. Apague a pasta 'auth_info' para reautenticar."
        );
      }
    } else if (connection === "open") {
      console.log(
        `✅ Bot conectado com sucesso! JID: ${jidNormalizedUser(
          sock.user?.id || ""
        )}`
      );
    }
      ...
      if (atendendo.has(from)) {
        ...
        const newTimeout = setTimeout(async () => {
          ...
          await sock.sendMessage(from, {
            text:
              "⏰ O atendimento foi encerrado por inatividade. Digite *MENU* para voltar ao início.",
          });
